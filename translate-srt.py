...
def traducir_archivo(archivo_entrada, lang, ext):
    ...
    archivo_destino = os.path.join(
        carpeta_destino, f"{nombre_archivo_sin_extension}_{lang}.{ext}")

    # Verificar si el archivo termina con "_es.srt"
    if nombre_archivo.endswith("_es.srt"):
        nombre_archivo_original = nombre_archivo.replace("_es.srt", ".srt")
        print(
            f"El archivo {nombre_archivo_original} ya ha sido traducido. Saltando...")
        return
    ...
    if len(texto) > 5000:
        ...
        traducciones = [translator.translate(
            bloque, dest=lang).text for bloque in bloques]
    ...
    print(
        f"El archivo traducido ha sido guardado como:\n{nombre_archivo_sin_extension}_{lang}.{ext}")
    print("=============================")
    print("")
...
carpeta_origen = input(
    "Introduce la ruta de la carpeta con los archivos de origen: ")

# Eliminar comillas al inicio y al final de la ruta (si existen)
...
archivos_a_traducir = [archivo for archivo in os.listdir(
    carpeta_origen) if archivo.lower().endswith('.srt')]
